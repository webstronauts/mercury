...
internals.serializeError = (error, message) => ({
  error: {
    code: error.expose ? decamelize(error.name).replace(/_error$/, '') : 'internal_server_error',
...
const errorHandler = module.exports = async function (app, options) {
  ...
  app.ready(err => {
    ...
    app.use((err, req, res, next) => {
      ...
      const error = HttpError.prototype.isPrototypeOf(err) ? err : new InternalServerError(err)
